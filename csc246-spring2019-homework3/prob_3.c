...
volatile int maxDifference = 0;

void *findMaxDifference(void *args) {
  maxDifference = results[0];
  for (int i = 1; i < counter; i++) {
    if (maxDifference < results[i]) {
      maxDifference = results[i];
    }
  }
  return NULL;
}
...
int main(int argc, char * argv[]) {
  ...
  if (pthread_join(producerThread, NULL) != 0) {
	...
	pthread_create(&calculateThread, NULL, findMaxDifference, NULL);
	pthread_join(calculateThread, NULL);
